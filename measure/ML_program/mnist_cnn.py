...
def customnet(_X, _weights, _biases, _dropout):
    ...
    dense1 = tf.nn.relu(tf.matmul(
        dense1, _weights['wd1']) + _biases['bd1'], name='fc1')  # Relu activation
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        if time.time() - time2 > 1:
            time2 = time.time()
            iter_interval = iter_temp2 - iter_temp1
            iter_temp1 = iter_temp2
    ...
    acc = sess.run(accuracy, feed_dict={x: mnist.test.images[
                   :256], y: mnist.test.labels[:256], keep_prob: 1.})
    print("Testing Accuracy:", acc)
    # assert(acc >= 0.6)
    print("Testing Results:", sess.run(tf.argmax(pred, 1),
                                       feed_dict={x: mnist.test.images[:4], keep_prob: 1.}))
